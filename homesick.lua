...
local function _fromG(...)
    for i = 1, select("#", ...) do
        local name = select(i, ...)
        local v = nil
        pcall(function() v = rawget(_G, name) end)
        if type(v) == "function" then return v end
        pcall(function() v = env[name] end)
        if type(v) == "function" then return v end
    end
    return nil
end
...
if not _ismouse1pressed then
    ...
    if ok and uis then
        ...
        _ismouse1pressed = function() return false end
    end
end
...
if not _ismouse2pressed then
    ...
    if ok and uis then
        ...
        _ismouse2pressed = function() return false end
    end
end
...
if not _iskeypressed then
    ...
    if ok and uis then
        ...
        _iskeypressed = function(vk)
            local kc = _vkToEnum[vk]
            if not kc then return false end
            local s, r = pcall(function() return uis:IsKeyDown(kc) end)
            return s and r or false
        end
    else
        _iskeypressed = function() return false end
    end
end
...
do
    local injections = {
        ...
        ismouse2pressed = _ismouse2pressed,
        iskeypressed    = _iskeypressed,
        keypress        = _keypress,
        keyrelease      = _keyrelease,
        isrbxactive     = _isrbxactive,
        setrobloxinput  = _setrobloxinput,
